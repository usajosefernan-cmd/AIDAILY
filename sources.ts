...
function getAIModelsList(preferred?: string): string[] {
  // Pool de modelos OpenRouter de ALTA CALIDAD e intercalados por proveedor para rotación óptima
  const whitelist = [
...
function saveCache(cachePath: string, items: NewsItem[]) {
  try {
    ...
    const cleanItems = items.filter(item => item.publishedAt.getTime() > cutoffTime);
    fs.writeFileSync(cachePath, JSON.stringify(cleanItems, null, 2), 'utf-8');
  } catch (e) {
