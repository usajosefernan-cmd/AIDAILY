...
export function writeJsonAtomic(filePath, data, createBackup = true) {
  ...
  // En Windows se implementa un bucle de reintentos síncrono por posibles bloqueos del descriptor de archivos
  let attempts = 15;
  while (attempts > 0) {
    try {
      ...
      attempts--;
      if (attempts === 0) {
        throw new Error(`[Atomic Write] No se pudo renombrar ${tempPath} a ${resolvedPath} tras 15 intentos. Detalle: ${err.message}`);
      }
      // Retardo síncrono de 100ms
